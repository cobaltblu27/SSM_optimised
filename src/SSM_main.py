...
class DILInew:
    ...
    def cal_preference(self, n_it):
        edge_class = defaultdict(lambda: defaultdict(int))

        for ltkbid in self.dEdgeUsedCount[n_it]:
            nClass = self.molinfo_df["class"][ltkbid]
            molobj = self.molinfo_df["molobj"][ltkbid]

            for edge, count in self.dEdgeUsedCount[n_it][ltkbid].items():
                a, b = map(int, edge.split('_'))
                frag = Chem.MolFragmentToSmiles(molobj, atomsToUse=[a, b])
                edge_class[frag][nClass] += count

        self.dEdgeClassDict[n_it] = {k: dict(v) for k, v in edge_class.items()}
    ...
    def get_individual_F(self, n_iter, n_iter_pref_df, ltkbid, mode=False):
        ...
        pref_row_sum = n_iter_pref_df.sum(axis=1)
        ...
        for edge in self.molinfo_df["molgraph"][ltkbid].edges():
            ...
            if self.molinfo_df["molgraph"][ltkbid][n1][n2]['order'] != 0:
                ...
                try:
                    probSeries = n_iter_pref_df[frag_smi] / pref_row_sum
        ...
        F[nonzero] = F[nonzero] / row_sums[nonzero]
    ...
    def rw_update_transitions(self, _T, _F, update_alpha):
        ...
        row_sums = _T.sum(axis=1, keepdims=True)
        nonzero = row_sums.flatten() > 0
        _T[nonzero] = _T[nonzero] / row_sums[nonzero]
...
def SMARTS_pattern_mining(valid_obj, train_X, k=2, epsilon=0.01, minimum_support_percent=2, minimum_entropy_cutoff=0.1):
    ...
    labels_pos = (train_labels == 1).values
    labels_neg = (train_labels == 0).values

    def entropy_from_match(match_result, return_support=False):
        pos_support = int((labels_pos & match_result).sum())
        neg_support = int((labels_neg & match_result).sum())
    ...
    # Pre-cache individual fragment match results (core optimization)
    n_frags = len(fragment_with_smarts)
    n_train = len(train_mols)
    match_cache = np.zeros((n_frags, n_train), dtype=bool)
    atom_counts = np.zeros(n_frags, dtype=int)
    print("Caching individual fragment matches...")
    for i, (frag_mol, frag_smarts) in enumerate(tqdm(fragment_with_smarts)):
        f = Chem.MolFromSmarts(frag_smarts)
        match_cache[i] = np.array([m.HasSubstructMatch(f) for m in train_mols])
        atom_counts[i] = frag_mol.GetNumAtoms()

    entropy_scores = np.array([entropy_from_match(match_cache[i])
    ...
    def backtrack(A, subset, subset_index, index, current_ent):
        ...
        if len(subset) > required_subset_len:
            return
        if len(subset) > 0:
            # Fast pre-filter: bitwise AND of individual matches
            combined_match = match_cache[subset_index[0]]
            for si in subset_index[1:]:
                combined_match = combined_match & match_cache[si]

            atom_cnt = int(atom_counts[subset_index].sum())
            if atom_cnt > maximum_num_atoms:
                return

            ent = entropy_from_match(combined_match, return_support=True)
            iterate_count += 1
