...
4단계 전체 워크플로우 (원본 SubDyve의 `mine_subgraph.sh`와 동일):
...
원본 SubDyve와 동일한 4단계를 수행합니다.

**샘플 데이터**: `examples/sample_targets/`에 원본 SubDyve의 실제 타겟 3개의 예제 데이터가 포함되어 있습니다 (실제 test_run 데이터에서 추출).
```
...
원본 SubDyve는 여러 타겟에 대해 4단계 파이프라인을 실행합니다:

```bash
# Step 1: 모든 타겟에 대해 기본 SSM 실행 (AUC 최적 iteration 찾기)
# 타겟명: 원본 SubDyve의 DUDE 단백질 타겟명 사용
...
# Step 2: 각 타겟의 최고 AUC iteration 수집
python src/collect.py \
  --threshold 0.9 \
  --targets ACES ADA EGFR

# Step 3: 최적 iteration에서 DiSC 패턴 마이닝
for target in ACES ADA EGFR; do
...
# Step 4: Top-N discriminative subgraph 추출
for target in ACES ADA EGFR; do
...
**참고**: 원본 SubDyve의 `mine_subgraph.sh`와 동일한 워크플로우입니다.
...
**중요 주의사항**:
- train 데이터와 test 데이터 모두 `label` 컬럼 사용 (내부적으로 `class`로 변환됨)
- **Train 데이터에 반드시 클래스 0과 1이 모두 포함되어야 합니다**
  - 한 클래스만 있으면 RandomForest 학습 시 `IndexError: index 1 is out of bounds` 발생
  - 예: 클래스 1만 20개 → RandomForest가 클래스 1만 학습 → `predict_proba()[:, 1]` 실패
  - 권장: 최소 각 클래스당 10개 이상
...
SSM_optimised는 다음 실행 가능한 스크립트를 제공합니다:

| Script | 용도 | 사용 시점 |
|--------|------|----------|
| `src/ssm_smiles.py` | 기본 SSM 알고리즘 실행 | 기본 실행 또는 Step 1 |
| `src/ssm_DISC.py` | DiSC refinement (최적 iteration 지정) | Multi-target 파이프라인 Step 3 |
| `src/collect.py` | 여러 타겟의 최고 AUC iteration 수집 | Multi-target 파이프라인 Step 2 |
| `src/clean_DISC.py` | Top-N discriminative subgraph 추출 | Multi-target 파이프라인 Step 4 |

**참고**: 원본 SubDyve와 동일한 4개의 스크립트를 제공합니다.
...
- **40/122 파일 FAIL**: DiSC CSV만 패턴 수 차이 (분류 성능에는 영향 없음)
...
- FAIL (40개): DiSC_2.csv, DiSC_3.csv만 패턴 수 차이 (해석용 출력, 분류에 무영향)
